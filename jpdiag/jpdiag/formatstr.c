...
/*++
	Routine Description:
		...
		of characters only.
--*/
static INT JpdiagsCompareString(
    __in PCWSTR Dst,
    __in PCWSTR Src,
	__in SIZE_T MaxCch
    )
...
--*/
static BOOL JpdiagsGetVariableBindingIndex(
	__in ULONG VarCount,
...
--*/
static HRESULT JpdiagsHandleVariable(
	__in ULONG VarCount,
	__in PFORMAT_VARIABLE Variables,
	...
	)
{
	...
	Translated = JpdiagsGetVariableBindingIndex( 
		VarCount,
		Variables,
		VariableNameBegin, 
	...
	)
{
	...
	_ASSERTE( Variables );
	_ASSERTE( Bindings );
	_ASSERTE( JpdiagpIsStringValid( FormatStringTemplate, 1, MAXWORD, FALSE ) );
	...
	if ( VarCount == 0 ||
		 ! Variables ||
		 ! Bindings ||
		 ! JpdiagpIsStringValid( FormatStringTemplate, 1, MAXWORD, FALSE ) ||
		 BufferSizeInChars == 0 ||
		 ! Buffer )
	{
		return E_INVALIDARG;
