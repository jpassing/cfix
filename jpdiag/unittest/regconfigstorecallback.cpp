...
} ParamAccessMode[] =
{
	...
	{ JPDIAG_CFGS_ACCESS_WRITE_USER | JPDIAG_CFGS_ACCESS_READ_USER },
	{ JPDIAG_CFGS_ACCESS_READ_MACHINE },
	{ JPDIAG_CFGS_ACCESS_WRITE_MACHINE },
	{ JPDIAG_CFGS_ACCESS_READ_MACHINE | JPDIAG_CFGS_ACCESS_WRITE_MACHINE },
	{ JPDIAG_CFGS_ACCESS_ALL },
	{ 0 }
};

typedef struct _PARAM_CALLBACK
{
	JPDIAG_CONFIGSTORE_UPDATE_CALLBACK Proc;
	ULONG Calls;
...
{
	PJPDIAG_CONFIGURATION_STORE Store = ( PJPDIAG_CONFIGURATION_STORE ) pv;
	UINT i, current = 0;
...
static DWORD CALLBACK TriggerUpdatesThreadProc( PVOID Unused )
{
	...
	while ( KeepGeneratingUpdates > 0 )
	{
		...
		TEST_HR( Store->WriteDwordSetting( 
			Store, L"X", L"X", JpdiagUserScope, Value++ ) );
	}
...
extern "C" void TestJpdiagCreateRegistryStoreCallback()
{
	//
