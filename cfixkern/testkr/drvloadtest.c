...
static void TestDrvWithDuplicateBefore()
{
	PCFIX_TEST_MODULE Mod;
	WCHAR Path[ MAX_PATH ];

	TEST( SUCCEEDED( StringCchCopy( Path, _countof( Path ), DriverDirectory ) ) );
	TEST( PathAppend( Path, L"testklib8.sys" ) );

	TEST( CFIX_E_DUP_BEFORE_ROUTINE == 
		CfixklCreateTestModuleFromDriver( Path, &Mod, NULL, NULL ) );
}

static void TestDrvWithDuplicateAfter()
{
	PCFIX_TEST_MODULE Mod;
	WCHAR Path[ MAX_PATH ];

	TEST( SUCCEEDED( StringCchCopy( Path, _countof( Path ), DriverDirectory ) ) );
	TEST( PathAppend( Path, L"testklib9.sys" ) );

	TEST( CFIX_E_DUP_AFTER_ROUTINE == 
		CfixklCreateTestModuleFromDriver( Path, &Mod, NULL, NULL ) );
}
...
CFIX_BEGIN_FIXTURE( DrvLoading )
	...
	CFIX_FIXTURE_ENTRY( TestDrvWithDuplicateBefore )
	CFIX_FIXTURE_ENTRY( TestDrvWithDuplicateAfter )
