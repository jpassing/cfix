...
#define _FSF	CFIX_FIXTURE_EXECUTION_SHORTCIRCUIT_FIXTURE_ON_FAILURE
#define _FSR	CFIX_FIXTURE_EXECUTION_SHORTCIRCUIT_RUN_ON_FAILURE
#define _FSS	CFIX_FIXTURE_EXECUTION_SHORTCIRCUIT_RUN_ON_SETUP_FAILURE
	...
	for ( RunIndex = 0; RunIndex < _countof( Runs ); RunIndex++ )
	{
		...
		for ( FixIndex = 0; FixIndex < Mod->FixtureCount; FixIndex++ )
		{
			if ( 0 != wcscmp( Mod->Fixtures[ FixIndex ]->Name, L"TestTsExecution" ) &&
				 0 != wcscmp( Mod->Fixtures[ FixIndex ]->Name, L"TestSingleTestCaseRun" ) )
			{
