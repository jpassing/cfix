...
#define TEST_LT( Expected, Actual )										\
	CFIXCC_ASSERT_NOT_EQUALS_MESSAGE( Expected, Actual, L"%d%s", 1, L"123" );	\
	CFIXCC_ASSERT_NOT_EQUALS( Expected, Actual );							\
	CFIXCC_ASSERT_LESS( Expected, Actual );								\
	CFIXCC_ASSERT_LESS_MESSAGE( Expected, Actual, L"%d%s", 1, L"123" );	\
	CFIXCC_ASSERT_GREATER( Actual, Expected );							\
	CFIXCC_ASSERT_GREATER_MESSAGE( Actual, Expected, L"%d%s", 1, L"123" );	\
	CFIXCC_ASSERT_LESS_OR_EQUAL( Expected, Actual );						\
	CFIXCC_ASSERT_LESS_OR_EQUAL_MESSAGE( Expected, Actual, L"%d%s", 1, L"123" );	\
	CFIXCC_ASSERT_GREATER_OR_EQUAL( Actual, Expected );					\
	CFIXCC_ASSERT_GREATER_OR_EQUAL_MESSAGE( Actual, Expected, L"%d%s", 1, L"123" );	\
...
CFIX_BEGIN_FIXTURE( FailedAssertions )
	...
	CFIX_FIXTURE_ENTRY( ExpectFailure< Fail01 > )
	CFIX_FIXTURE_ENTRY( ExpectFailure< Fail02 > )
	CFIX_FIXTURE_ENTRY( ExpectFailure< Fail03 > )
CFIX_END_FIXTURE()

CFIX_BEGIN_FIXTURE( Log )
