	...
	)
{
	...
	//
	// Assemble ExecutionFlags.
	//
	ExecutionFlags = 0;
	if ( SearchCtx->State->Options->ShortcutFixtureOnFailure )
	{
		ExecutionFlags |= CFIX_FIXTURE_EXECUTION_SHORTCUT_FIXTURE_ON_FAILURE;
	}
	if ( SearchCtx->State->Options->ShortcutRunOnFailure )
	{
		ExecutionFlags |= CFIX_FIXTURE_EXECUTION_SHORTCUT_RUN_ON_FAILURE;
	}
	if ( SearchCtx->State->Options->ShortcutRunOnSetupFailure )
	{
