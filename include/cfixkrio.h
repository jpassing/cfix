...
typedef struct _CFIXKR_IOCTL_CALL_ROUTINE_REQUEST
{
	...
	//
	// TLS value. The value is piggybacked for each call as the
	// reflector itself does not maintain per-fixture
	// state.
	//
	// N.B. TlsValue may contain a pointer, which, from a security
	// perspective is dubious. Yet, as the user was able to load
	// a driver in the first place, and thus already has complete control 
	// over the system anyway, this may be ignored in practice.
	//
	ULONGLONG TlsValue;
